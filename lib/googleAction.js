...
let askUrgency = (app) =>{
    app.ask(
        app.buildRichResponse()
            .addSimpleResponse("Select Urgency")
            .addSuggestions(["High", "Medium", 'Low'])
    )
};
...
let getIncident = (app) =>{
    let incidentNo = app.body_.result.parameters.incidentNo.toString().toUpperCase();
    if(incidentNo.indexOf('INC') != 0) {
      ...
      let resObj =  {
        ...
        "messages" :[{
            "type": "simple_response",
            "platform": "google",
            "textToSpeech": "Please find status of "+ req.body.result.parameters.incidentNo.toUpperCase()+" below."
        }],      
        followupEvent :{
            "name" : "getIncident",
      ...
      if(body && body["result"]){
        let resObj = {
          ...
          "messages": [
            ...
            },
            {
                "type": "basic_card",
                "platform": "google",
                "title": "Incident No : "+ req.body.result.parameters.incidentNo.toUpperCase(),
                "subtitle": "Status : " +data.status[body.result[0].state] ? data.status[body.result[0].state] : "CLOSED",
                "formattedText": "",
                "image": {
        ...
        let resObj =  {
          ...
          "messages" :[{
            "type": "simple_response",
            "platform": "google",
            "textToSpeech": "Invalid Incident No. Please Check and try again."
        }],
          followupEvent :{
                  "name" : "getIncident",
                  "data" : {}
...
module.exports.subCategoryAction = askSubCCategory;