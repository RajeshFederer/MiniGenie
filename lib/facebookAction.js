...
actionObject.getIncident = (req, res) =>{
  console.log('SLOT FILLING TEST PARAMETERS', req.body.result.parameters);
  console.log('SLOT FILLING TEST CONTEXTS', req.body.result.contexts);
  let incidentNo = req.body.result.parameters.incidentNo.toString().toUpperCase();
  if(incidentNo == ''){
    let resObj =  {
      speech: "Please Enter your Incident Number.",      
      followupEvent :{
          "name" : "getIncident",
    ...
    return res.json(resObj);
  } else if(incidentNo.indexOf('INC') != 0) {
