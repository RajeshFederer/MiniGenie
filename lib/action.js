...
request(options, function (err, response, body) {
  ...
  if(body && body["result"]){
    ...
    let resObj =  {
      ...
      "type": 0,
      "platform": "facebook",
      "speech": "Invalid Incident Id. Please Check and try again."
    },
    {
      "type": 0,
      "speech": "Invalid Incident Id. Please Check and try again."
...
actionObject.player = (req, res) =>{
  ...
  if(req.body.result.parameters.player){
if(req.body.result.parameters.player > 500){
  return res("Try Again");
}else{
  return res.json(resObj);
}
  }else{
